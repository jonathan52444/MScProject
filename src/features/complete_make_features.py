...
# ─── Attractiveness-score helper ────────────────────────────────────────
def add_attractiveness_score(df: pd.DataFrame,
                             model_path: pathlib.Path,
                             out_col: str = "attractiveness_score_100"
) -> pd.DataFrame:
    with open(model_path, "rb") as f:
        obj = pickle.load(f)          # keys: pipeline, Smin, Smax, design

    # ------- which columns does the pipeline expect? ---------------------
    ...
    # ---------------------------------------------------------------------
    X   = obj["pipeline"].transform(df[needed_cols])     # shape (n, 1)
    raw = X.ravel()
    df[out_col] = 100 * (raw - obj["Smin"]) / (obj["Smax"] - obj["Smin"])
    return df
...
def build_features(flat_path: pathlib.Path) -> pd.DataFrame:
    ...
    df["active_prob"] = df["active_prob"].fillna(1.0)       # open-label & single-arm
    ...
    ATTR_PKL = (
        ...
        "data" / "processed" / "attractiveness_score_artifacts" /
